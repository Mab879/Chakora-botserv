...
sub svs_ns_ghost {
	...
	$dele .= 'serv_logout(\''.$nick.'\'); event_logout(\''.$nick.'\', \''.uidInfo($nick, 9).'\'); serv_kill(\'nickserv\', \''.$nick.'\', \'Killed (NickServ (GHOST command used by '.uidInfo($user, 1).'!'.uidInfo($user, 2).'@'.uidInfo($user, 4).'))\'); ';
	$dele .= '1; ';
