# protocol/ratbox by The Chakora Project. Link with ircd-ratbox.
#
...
$Chakora::MODULE{protocol}{name} = 'protocol/Ratbox';
...
sub irc_connect {
	if (length(config('me', 'sid')) != 3) {
		...
		send_sock("CAPAB :QS KLN UNKLN ENCAP EX CHW IE KNOCK SAVE UID SERVICES RSFNC MLOCK TB EOPMOD BAN");
...
sub serv_add {
	...
	send_sock(":".svsUID('chakora::server')." UID ".$nick." 0 ".time()." ".$modes." ".$user." ".$host." 0.0.0.0 ".$ruid." :".$real);
...
# Handle CHGHOST
sub serv_chghost {
	# This doesn't exist in ircd-ratbox.
}
...
sub raw_uid {
	...
	event_uid($ruid, $rex[2], $rex[6], $rex[7], $rex[7], $rex[8], substr($rex[0], 1));
    	my $svs = 'operserv';
    	if (module_exists("global/main")) {
        	$svs = 'global';
    	}
