...
sub irc_connect {
    ...
    send_sock(
        "SERVER " . config( 'me', 'name' ) . " 1 :" . config( 'me', 'info' ) );
    raw_bursting();
}
...
sub serv_mode {
    my ( $svs, $chan, $modes ) = @_;
    send_sock(
        ":" . svsUID('chakora::server') . " MODE " . $chan . " " . $modes );
}
...
sub raw_bursting {
    ...
    serv_add(
        ...
        "+oS",
        config( 'chanserv', 'real' )
    );
    serv_add(
        svsUID('hs'),
        config( 'hostserv', 'user' ),
        config( 'hostserv', 'nick' ),
        config( 'hostserv', 'host' ),
        "+oS",
        config( 'hostserv', 'real' )
    );
    serv_add(
        svsUID('ms'),
        config( 'memoserv', 'user' ),
        config( 'memoserv', 'nick' ),
        config( 'memoserv', 'host' ),
        "+oS",
        config( 'memoserv', 'real' )
    );
    serv_add(
        svsUID('ns'),
...
# [IRC] NICK lol2 1 1282320638 MattB localhost dev.matt-tech.info 0 +iowghaAxNt lol fwAAAQ== :Matthew - Laptop
        ...
        $uid{$ruid}{'host'}  = $rex[5];
        $uid{$ruid}{'pnick'} = 0;
        event_uid( $ruid, $rex[1], $rex[4], $rex[5], $rex[9],
            decode_base64( $rex[10] ) );
