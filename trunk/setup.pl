...
if ( -d 'etc' ) {
    ...
    my $name = prompt(
        'x',
        'What is the linkname for this server?',
        'eg. some.server.tld', ''
    );
    print("\n");
    ...
    my $info = prompt(
        'x', 'What should be the description for this server?',
    ...
    my $sid = prompt(
        'x',
        'What should be the SID for this server?',
        'If your using inspircd or charybdis, this needs to be 3 characters.',
        '34R'
    );
    print("\n");
    $conf->append( "\tsid = " . $sid . "\n" );
    ...
    my $ircd = prompt(
        'a',                   'What is the IRCd of the remote server?',
        'inspircd, charybdis', 'inspircd'
    );
    print("\n");
    ...
    my $host = prompt(
        'x',
        'What is the hostname of the remote server?',
        'eg. some.server.tld', ''
    );
    print("\n");
    ...
    my $vhost = prompt(
        'x',
        'What should be the vHost (ip to bind to) of this server?',
        'Mainly for multi-homed hosts.', '0.0.0.0'
    );
    print("\n");
    ...
    my $pass =
      prompt( 'x', 'What is the linkpass for this server?', '', 'linkage' );
    print("\n");
    ...
    my $name = prompt(
        'x', 'What is the name of the network we are linking to?',
        '',  'technoirc'
    );
    print("\n");
    $conf->append( "\t" . 'name = "' . $name . "\"\n" );
    ...
    my $admin =
      prompt( 'x', 'Who is the admin of these services?', '', 'starcoder' );
    print("\n");
    ...
    my $allow_taint = prompt(
        'y',
        'Do you want to allow tainting?',
        'Allowing tainting loses your support rights', 'n'
    );
    ...
    my $key = prompt(
        'x',
        'What do you want to use for the encryption key',
'This needs to be unique to your IRC network, make sure this strong and random',
        ''
    );
    $conf->append( "\t" . 'key = "' . $key . "\"\n" );
    $conf->append("}\n\n");
    ...
    my $user =
      prompt( 'x', 'What username should ' . $svs . ' use?', '', $svs );
    print("\n");

    my $host = prompt(
        'x', 'What host should ' . $svs . ' use?',
        '',  $name . "/Services/" . $svs
    );
    print("\n");

    my $real = prompt(
        'x', 'What realname should ' . $svs . ' use?',
        '',  "Channel Services"
    );
    print("\n");
    ...
    my $real = prompt( 'x', 'What realname should ' . $svs . ' use?',
        '', "Oper Services" );
    print("\n");
    ...
    my $sras = prompt(
        'x',
        'Who should have SRA Status?',
        'Separate nicks with spaces.',
        "starcoder MattB"
    );
    print("\n");
    ...
    $conf->append("xmlrpc {\n");
    my $xmlrpc_use = prompt(
        'y',
        'Do you want to use the XML-RPC',
        'Good for web services and remote control of services', 'y'
    );
    print("\n");
    $conf->append( "\tuse = " . $xmlrpc_use . "\n" );

    if ($xmlrpc_use) {
        my $xmlrpc_host = prompt(
            'x',
            'What IP should the XML-RPC bind to?',
            'Mainly for multi-homed hosts.', '0.0.0.0'
        );
        print("\n");
        my $xmlrpc_port =
          prompt( 'x', 'What port should the XML-RPC bind to?', '', '8080' );
        print("\n");
